...
function CustomTooltip({ active, payload, label }: TooltipProps<number, string>) {
  if (active && payload && payload.length) {
    return (
      <div className="rounded-lg border border-red-500/30 bg-[#0d0d0d]/95 p-4 shadow-xl backdrop-blur-sm">
        <div className="mb-3 flex items-center gap-2">
          <div className="h-1.5 w-1.5 rounded-full bg-red-500 animate-pulse" />
          <p className="font-mono text-xs uppercase tracking-wider text-red-400">{label}</p>
        </div>
    ...
    <div className="group relative overflow-hidden rounded-lg border border-red-900/30 bg-gradient-to-br from-[#0d0d0d] to-[#111111] p-5">
      ...
      <div className="absolute left-0 top-0 h-12 w-12">
        <svg viewBox="0 0 48 48" className="h-full w-full">
          <path d="M0,0 L16,0 L16,2 L2,2 L2,16 L0,16 Z" fill="rgba(220, 38, 38, 0.3)" />
          <path d="M0,20 L8,20 L8,22 L0,22 Z" fill="rgba(220, 38, 38, 0.15)" />
        </svg>
      </div>
      <div className="absolute bottom-0 right-0 h-12 w-12 rotate-180">
        <svg viewBox="0 0 48 48" className="h-full w-full">
          <path d="M0,0 L16,0 L16,2 L2,2 L2,16 L0,16 Z" fill="rgba(220, 38, 38, 0.2)" />
        </svg>
      </div>
      ...
      <div className="relative mb-4 flex items-center justify-between">
        ...
        <div className="flex rounded-lg border border-red-900/40 bg-[#080808] p-1">
      ...
      <div className="h-64">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data}>
            ...
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(220, 38, 38, 0.1)" vertical={false} />
            <XAxis 
              dataKey="date" 
              stroke="#444" 
              tick={{ fill: '#555', fontSize: 11, fontFamily: 'monospace' }}
              axisLine={{ stroke: 'rgba(220, 38, 38, 0.2)' }}
              tickLine={{ stroke: 'rgba(220, 38, 38, 0.1)' }}
            />
            <YAxis 
              stroke="#444" 
              tick={{ fill: '#555', fontSize: 11, fontFamily: 'monospace' }}
