...
export function Sidebar() {
  ...
  return (
    <aside className="flex w-64 flex-col border-r border-red-900/30 bg-[#050505] relative overflow-hidden">
      {/* Animated background grid */}
      <div className="absolute inset-0 opacity-20">
        <svg className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <pattern id="sidebar-grid" width="40" height="40" patternUnits="userSpaceOnUse">
              <path d="M 40 0 L 0 0 0 40" fill="none" stroke="rgba(220, 38, 38, 0.15)" strokeWidth="0.5" />
            </pattern>
          </defs>
      ...
      <div className="absolute left-0 top-0 w-32 h-32 pointer-events-none z-10">
        <svg viewBox="0 0 100 100" className="w-full h-full">
          <path d="M0,0 L40,0 L40,2 L2,2 L2,40 L0,40 Z" fill="rgba(220, 38, 38, 0.4)" />
          <path d="M0,10 L30,10 L30,12 L2,12 L2,30 L0,30 Z" fill="rgba(220, 38, 38, 0.25)" />
          <path d="M10,0 L10,20 L12,20 L12,0 Z" fill="rgba(220, 38, 38, 0.2)" />
          <circle cx="25" cy="25" r="2" fill="rgba(220, 38, 38, 0.5)" />
        </svg>
      </div>

      {/* Logo Section */}
      <div className="relative flex flex-col items-center border-b border-red-900/30 py-6 z-10">
        <div className="relative">
          <div className="relative h-20 w-20 rounded-full border-2 border-red-600/50 overflow-hidden">
            <Image
              src="/images/logo.jpg"
              alt="Trebol Logo"
              fill
        ...
        <p className="font-mono text-xs tracking-[0.3em] text-red-500">DE TREBOL</p>
      </div>

      {/* Navigation */}
      <nav className="flex-1 p-3 z-10 overflow-y-auto scrollbar-thin scrollbar-track-transparent scrollbar-thumb-red-900/50">
        <ul className="space-y-1">
            ...
            <Link
              ...
              className={cn(
                ...
                pathname === '/dashboard'
                  ...
                  : 'text-gray-500 hover:bg-red-900/10 hover:text-gray-300 border-l-2 border-transparent'
              )}
            >
              <LayoutDashboard className="h-4 w-4" />
              Dashboard
              {pathname === '/dashboard' && (
                <div className="absolute right-2 top-1/2 h-1.5 w-1.5 -translate-y-1/2 rounded-full bg-red-500 shadow-lg shadow-red-500/50" />
              )}
            </Link>
          </li>
          ...
          <li>
            ...
            {gatesOpen && (
              <div className="mt-1 ml-2 pl-4 border-l border-red-900/30 space-y-1">
                {/* Ver Todos Link */}
                <Link
                  ...
                  className={cn(
                    ...
                    pathname === '/dashboard/gates'
                      ...
                      : 'text-gray-500 hover:bg-red-900/10 hover:text-gray-300'
                  )}
                >
            ...
            <button
              ...
              className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-xs text-gray-500 hover:bg-red-900/10 hover:text-gray-300 transition-all duration-200 cursor-pointer"
            >
              <CategoryIcon className="h-3 w-3" />
              <span className="flex-1 text-left">{categoryLabels[category]}</span>
                        {categoryGates.length > 0 && (
                          <>
                            <span className="text-red-500/70 text-[10px]">({categoryGates.length})</span>
                            ...
                            <Link
                              ...
                              className={cn(
                                ...
                                pathname === `/dashboard/gates/${gate.id}`
                                  ? 'bg-red-600/20 text-red-400'
                                  : 'text-gray-600 hover:bg-red-900/10 hover:text-gray-400'
                              )}
                            >
      ...
      {/* Bottom decoration */}
      <div className="relative h-36 overflow-hidden border-t border-red-900/30 z-10">
        <div className="absolute inset-0 bg-gradient-to-t from-red-600/20 via-red-900/10 to-transparent" />
        <Image
        ...
        />
        <div className="absolute inset-0 bg-[linear-gradient(transparent_50%,rgba(220,38,38,0.02)_50%)] bg-[length:100%_4px]" />
      </div>
    </aside>
  )
}
