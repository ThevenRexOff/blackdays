...
const actionColors = {
  ...
  ELIMINO: 'bg-red-500/80 shadow-red-500/50',
  'INICIO SESION': 'bg-violet-500/80 shadow-violet-500/50',
}

export function RecentActivity() {
  return (
    <div className="group relative rounded-xl border border-red-500/20 bg-gradient-to-br from-black/90 via-[#0a0808] to-black/90 p-5 backdrop-blur-xl overflow-hidden">
      {/* Cyber corner accents */}
      <div className="absolute top-0 left-0 w-16 h-16">
        <div className="absolute top-0 left-0 w-full h-[2px] bg-gradient-to-r from-red-500 to-transparent"></div>
        <div className="absolute top-0 left-0 w-[2px] h-full bg-gradient-to-b from-red-500 to-transparent"></div>
      </div>
      <div className="absolute top-0 right-0 w-16 h-16">
        <div className="absolute top-0 right-0 w-full h-[2px] bg-gradient-to-l from-red-500 to-transparent"></div>
        <div className="absolute top-0 right-0 w-[2px] h-full bg-gradient-to-b from-red-500 to-transparent"></div>
      </div>
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <div className="flex-1 h-[1px] bg-gradient-to-r from-red-500/50 to-transparent ml-2"></div>
      </div>
      
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead>
            <tr className="border-b border-red-500/20 text-left text-xs uppercase text-gray-500">
          ...
          <tbody className="text-sm">
            {activities.map((activity, index) => (
              <tr 
                key={activity.id} 
                className="border-b border-red-500/10 transition-all duration-300 hover:bg-red-500/5"
                style={{ animationDelay: `${index * 100}ms` }}
              >
                <td className="py-3 pr-4">
                  <div className="flex items-center gap-3">
                    <div className="relative">
                      <div className="absolute -inset-1 bg-red-500/30 rounded-full blur-sm"></div>
                      <div className="relative h-9 w-9 overflow-hidden rounded-full border-2 border-red-500/50 ring-2 ring-red-500/20">
                        <Image
                          src={activity.avatar}
                          alt={activity.user}
                          fill
                ...
                <td className="py-3">
                  <Button variant="ghost" size="icon" className="h-8 w-8 text-gray-500 hover:text-red-500 hover:bg-red-500/10 transition-colors">
      ...
      <div className="mt-5 flex justify-center">
        <Button 
          variant="ghost" 
          className="group/btn relative px-6 py-2 text-red-500 hover:text-white hover:bg-transparent overflow-hidden"
        >
          <span className="absolute inset-0 border border-red-500/50 rounded-md"></span>
          <span className="absolute inset-0 bg-gradient-to-r from-red-600 to-red-500 rounded-md opacity-0 group-hover/btn:opacity-100 transition-opacity"></span>
