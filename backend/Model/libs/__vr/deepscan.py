...
_JS_FULL_CHECK = """() => {
    ...
    try { if (window.Conekta) pay.push('Conekta'); } catch(e) {}
    try { if (window.OpenPay || window.openpay) pay.push('OpenPay'); } catch(e) {}
    try { if (window.Clip) pay.push('Clip'); } catch(e) {}
    try { if (window.KueskiPay || window.Kueski) pay.push('Kueski Pay'); } catch(e) {}
    try { if (window.Checkout && window.Checkout.configure) pay.push('Checkout.com'); } catch(e) {}
    try { if (window.Frames && window.Frames.isCardValid) pay.push('Checkout.com'); } catch(e) {}
    try { if (window.WePay) pay.push('WePay'); } catch(e) {}
    try { if (window.Spreedly) pay.push('Spreedly'); } catch(e) {}
    try { if (window.klarnaAsyncCallback) pay.push('Klarna Checkout'); } catch(e) {}
    try { if (window.paidy) pay.push('Paidy'); } catch(e) {}
    try { if (window.Cardinal) pay.push('Cardinal Commerce'); } catch(e) {}
    try { if (window.KlarnaOnsiteService || window._klarnaCheckout) pay.push('Klarna Checkout'); } catch(e) {}
    try { if (window.CheckoutWebComponents) pay.push('Checkout.com'); } catch(e) {}
    try { if (window.OffAmazonPayments || window.onAmazonPaymentsReady || window.AmazonPayments) pay.push('Amazon Pay'); } catch(e) {}
...
async def _deep_scan_async(url: str, ua: str, timeout_ms: int) -> Dict:
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            except: pass  # non-fatal — continue with what we have
        except: pass
        ...
        #//* 2b) Scan loaded network resources (catches lazy-loaded payment scripts)
        try:
            extra_urls = await page.evaluate("""() => {
                try { return performance.getEntriesByType('resource').map(r => r.name); }
                catch(e) { return []; }
            }""")
            for u in extra_urls:
                ul = u.lower()
                for domain, name in {
                    'js.stripe.com': 'Stripe', 'checkout.stripe.com': 'Stripe',
                    'conekta.io': 'Conekta', 'openpay.mx': 'OpenPay',
                    'cdn.checkout.com': 'Checkout.com', 'js.checkout.com': 'Checkout.com',
                    'x.klarnacdn.net': 'Klarna Checkout',
                    'js.braintreegateway.com': 'Braintree',
                    'pay.google.com': None, 'applepay.cdn-apple.com': None,
                    'klickcheckout.com': 'Klarna Checkout',
                }.items():
                    if domain in ul and name:
