...
class WebPage(BaseWebPage):
    def _parse_html(self):
        ...
        #//* Extract form action URLs — gateways often reveal themselves here
        self.forms = [form.get('action', '') for form in soup.findAll('form') if form.get('action')]

        #//* Extract preconnect/dns-prefetch hints — CDN origins before JS loads
        self.preconnects = [
            link['href'] for link in soup.findAll('link', rel=True, href=True)
            if any(r in (link.get('rel') or []) for r in ('preconnect', 'dns-prefetch'))
        ]

        #//* Extract data-* payment key attributes from forms/divs
        _pay_data_attrs = ('data-stripe-key', 'data-braintree-client', 'data-adyen-environment',
                           'data-mercadopago-key', 'data-public-key', 'data-conekta-key',
                           'data-openpay-id', 'data-checkout-public-key')
        self.pay_data = {}
        for attr in _pay_data_attrs:
            el = soup.find(attrs={attr: True})
            if el:
                self.pay_data[attr] = el.get(attr, '')
