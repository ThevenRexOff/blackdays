...
class Flexxe:
    ...
    _CSP_PAYMENT_MAP: Dict[str, str] = {
        ...
        'openpay.mx':            'OpenPay',
        'clip.mx':               'Clip',
        'kueski.com':            'Kueski Pay',
        'pagali.mx':             'Pagali',
        'checkout.com':          'Checkout.com',
        'cdn.checkout.com':      'Checkout.com',
        'x.klarnacdn.net':       'Klarna Checkout',
    }

    def _detectFromCSP(self, webpage: IWebPage) -> Set[str]:
        """Parse CSP header to find whitelisted payment processor domains."""
        found: Set[str] = set()
        csp = webpage.headers.get('content-security-policy', '').lower()
        if not csp:
            return found
    ...
    def _scanJsBundles(self, webpage: IWebPage, ua: str) -> Set[str]:
        ...
        #//* Limit to 5 bundles max, download in parallel
        resolved = []
        ...
        with ThreadPoolExecutor(max_workers=5) as pool:
            futures = {pool.submit(_fetch_bundle, u): u for u in resolved}
    ...
    _PRECONNECT_MAP: Dict[str, str] = {
        'js.stripe.com':         'Stripe',
        'checkout.stripe.com':   'Stripe',
        'js.braintreegateway.com':'Braintree',
        'cdn.checkout.com':      'Checkout.com',
        'x.klarnacdn.net':       'Klarna Checkout',
        'sdk.mercadopago.com':   'MercadoPago',
        'cdn.conekta.io':        'Conekta',
        'resources.openpay.mx':  'OpenPay',
        'cdn.adyen.com':         'Adyen',
        'js.paypal.com':         'PayPal',
        'cdn.braintree-api.com': 'Braintree',
    }

    def _scanPreconnects(self, webpage: IWebPage) -> Set[str]:
        """Detect gateways via <link rel=preconnect/dns-prefetch> hints."""
        found: Set[str] = set()
        for url in getattr(webpage, 'preconnects', []):
            ul = url.lower()
            for domain, name in self._PRECONNECT_MAP.items():
                if domain in ul:
                    found.add(name)
        return found

    _PAY_DATA_ATTR_MAP: Dict[str, str] = {
        'data-stripe-key':        'Stripe',
        'data-braintree-client':  'Braintree',
        'data-adyen-environment': 'Adyen',
        'data-mercadopago-key':   'MercadoPago',
        'data-conekta-key':       'Conekta',
        'data-openpay-id':        'OpenPay',
        'data-checkout-public-key':'Checkout.com',
    }

    def _scanPayData(self, webpage: IWebPage) -> Set[str]:
        """Detect gateways from data-* attributes on payment elements."""
        found: Set[str] = set()
        for attr, name in self._PAY_DATA_ATTR_MAP.items():
            if attr in getattr(webpage, 'pay_data', {}):
                found.add(name)
        return found

    _FORM_PAYMENT_DOMAINS: Dict[str, str] = {
        'authorize.net':    'Authorize.Net',
        'sim.authorize.net':'Authorize.Net',
        'worldpay.com':     'Worldpay',
        'secure.worldpay':  'Worldpay',
        'paypal.com':       'PayPal',
        'payflow':          'PayPal',
        'conekta.io':       'Conekta',
        'openpay.mx':       'OpenPay',
        'checkout.com':     'Checkout.com',
        'secure.2checkout': 'Checkout.com',
        'pay.google.com':   None,
    }

    def _scanFormActions(self, webpage: IWebPage) -> Set[str]:
        """Detect gateways that reveal themselves via <form action> URLs."""
        found: Set[str] = set()
        for action in getattr(webpage, 'forms', []):
            al = action.lower()
            for domain, name in self._FORM_PAYMENT_DOMAINS.items():
                if domain in al and name:
                    found.add(name)
        return found
    ...
    def analyzeWithCategories(self, webpage:IWebPage, ua:str = '') -> Dict[str, Any]:
        ...
        extra_pay.update(self._scanFormActions(webpage))
        extra_pay.update(self._scanPreconnects(webpage))
        extra_pay.update(self._scanPayData(webpage))
...
def analyze(url:str, useragent:str = '') -> Dict[str, Any]:
    global _FLEXXE_INSTANCE
    try:
        if _FLEXXE_INSTANCE is None:
            _FLEXXE_INSTANCE = Flexxe.makeObject()
        flexxe = _FLEXXE_INSTANCE
        headers = {}
        if not useragent:
            useragent = _UA_POOL[hash(url) % len(_UA_POOL)]
