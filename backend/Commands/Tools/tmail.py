...
# ══════════════════════════════════════════════════════════════════════════
import time, json, tempfile, os, html as _html_mod
    ...
    try:
        ...
        bot.editMessage(message_id=update.message_id, reply_markup=bot.replyMarkup(*rows), text=(
            ...
            + ('' if not inbox.messages else
               '\n' + '\n'.join(
                   f"{E_LETTER} [{i+1}] <code>{_html_mod.escape(m['from'][:30])}</code> — {_html_mod.escape(m['subject'][:40])}"
...
def tmail_read(bot, update, gestion) -> None:
    try:
        ...
        bot.editMessage(message_id=update.message_id, reply_markup=bot.replyMarkup(*rows), text=(
            ...
            f"{E_LETTER} {bi('From')}: <code>{_html_mod.escape(str(msg.sender))}</code>\n"
            f"{E_LETTER} {bi('Subject')}: <code>{_html_mod.escape(str(msg.subject))}</code>\n"
            f"{E_BOLT} {bi('Date')}: <code>{_html_mod.escape(str(msg.date))}</code>\n{DIV_STARS}\n"
            f"<code>{_html_mod.escape(str(msg.text)[:3000])}</code>\n{DIV_STARS}"
        ))
