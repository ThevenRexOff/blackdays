...
def gateCmd(bot, update, gestion):
    try:
        ...
        if not success:
            resp_msg = _html_mod.escape(str(result.get('response') or result.get('message') or 'Gate error'))
            txt = f"{bot.bi('Wrong Data')} [ ⚠️ ]\n🍸 {bot.bi('Raise')}: <code>{resp_msg}</code>"
        ...
        card = (
            ...
            f"📧 {bot.bi('Email')}: <code>{_html_mod.escape(str(result.get('email', 'N/A')))}</code>\n"
            f"🔑 {bot.bi('Password')}: <code>{_html_mod.escape(str(result.get('password', 'N/A')))}</code>\n{DIV}\n"
