# Pure gate motor for 'mj' — no Telegram-bot dependency.
...
def _flow(num, mes, ano, cvv):
    ...
    fn, ln = _name()
    payload = {'token': tk_id, 'email': _email(), 'first_name': fn, 'last_name': ln, 'password': _password(), 'package_name': 'Mensual', 'plan_id': '1', 'coupon_code': ''}
    r3 = sess.post('https://register.ohjazz.tv/subscription_trial', headers=hdrs, json=payload, allow_redirects=False, timeout=20)
    r3t = r3.text
    try:
